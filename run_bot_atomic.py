...
async def create_incident_record_atomic(self, **values):
    """Create the incident and its creation ledger event in one transaction."""
    if not self.db_pool:
        ...
        async with self.db_pool.acquire() as conn:
            async with conn.transaction():
                await conn.execute(
                    ...
                    values["location"], values["situation"], values["priority"],
                )
                await conn.execute(
                    """
...
async def delete_incident_after_card_failure(self, channel_id):
    """Roll back DB creation if Discord could not create the incident card."""
    if not self.db_pool:
        return
    try:
        async with self.db_pool.acquire() as conn:
            async with conn.transaction():
                incident = await conn.fetchrow(
                    "SELECT guild_id,incident_number FROM rescue_incidents WHERE channel_id=$1 FOR UPDATE",
                    channel_id,
                )
                if not incident:
                    return
                await conn.execute(
                    "DELETE FROM rescue_incident_events WHERE guild_id=$1 AND incident_number=$2",
                    incident["guild_id"], incident["incident_number"],
                )
...
async def incident_submit_atomic(self, interaction):
    """Create a complete incident or clean up partial Discord/DB resources."""
    ...
    try:
        ...
        created, _reason = await core.bot.create_incident_record(
