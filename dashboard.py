...
<title>{esc(title)} · SC Rescue</title>
<style>
    ...
    async with pool.acquire() as conn:
        ...
        active = await conn.fetch(
            """SELECT incident_number,callsign,service,location,priority,status,primary_responder_id,created_at
               FROM rescue_incidents WHERE guild_id=$1 AND status<>'closed'
               ORDER BY CASE priority WHEN 'critical' THEN 1 WHEN 'urgent' THEN 2 ELSE 3 END, incident_number ASC LIMIT 50""",
            guild_id,
        )
        history = await conn.fetch(
            """SELECT incident_number,callsign,service,priority,primary_responder_id,created_at,responded_at,closed_at
               FROM rescue_incidents WHERE guild_id=$1 AND status='closed'
        ...
        stats = await conn.fetchrow(
            """SELECT COUNT(*) AS total,
                      ...
                      AVG(EXTRACT(EPOCH FROM (responded_at-created_at))) FILTER (WHERE responded_at IS NOT NULL) AS avg_response
...
async def guild_dashboard(request: Request, guild_id: int, saved: int = 0):
    ...
    def channel_select(name, selected):
        opts = '<option value="">Not configured</option>'
        opts += "".join(option(c["id"], f'#{c["name"]}', int(c["id"]) == selected) for c in text_channels)
    ...
    active_rows = "".join(
        f'<tr><td>RESCUE-{r["incident_number"]:04d}</td><td><span class="pill {"p1" if r["priority"]=="critical" else "p2" if r["priority"]=="urgent" else "p3"}">{esc(PRIORITIES.get(r["priority"], r["priority"]))}</span></td><td>{esc(STATUSES.get(r["status"], r["status"]))}</td><td>{esc(SERVICES.get(r["service"], r["service"]))}</td><td>{esc(r["callsign"])}</td><td>{esc(r["location"])}</td><td>{"<@"+str(r["primary_responder_id"])+">" if r["primary_responder_id"] else "Unassigned"}</td></tr>'
        for r in active
    ) or '<tr><td colspan="7" class="muted">No active incidents.</td></tr>'

    history_rows = "".join(
        f'<tr><td>RESCUE-{r["incident_number"]:04d}</td><td>{esc(SERVICES.get(r["service"], r["service"]))}</td><td>{esc(PRIORITIES.get(r["priority"], r["priority"]))}</td><td>{esc(r["callsign"])}</td><td>{duration((r["responded_at"]-r["created_at"]).total_seconds()) if r["responded_at"] else "—"}</td><td>{format_dt(r["closed_at"])}</td></tr>'
        for r in history
    ...
    body = f"""
{notice}
<div class="section-nav"><a href="#overview">Overview</a><a href="#active">Active Incidents</a><a href="#history">History</a><a href="#config">Configuration</a></div>
<div class="grid" id="overview">
<div class="card span3"><div class="label">Total Incidents</div><div class="metric">{stats['total']}</div></div>
<div class="card span3"><div class="label">Active</div><div class="metric">{stats['active']}</div></div>
<div class="card span3"><div class="label">Completed</div><div class="metric">{stats['closed']}</div></div>
<div class="card span3"><div class="label">Avg Claim Time</div><div class="metric">{duration(stats['avg_response'])}</div></div>
<div class="card span12" id="active"><h2>Active Incidents</h2><div style="overflow:auto"><table><thead><tr><th>Incident</th><th>Priority</th><th>Status</th><th>Service</th><th>Callsign</th><th>Location</th><th>Primary</th></tr></thead><tbody>{active_rows}</tbody></table></div></div>
<div class="card span12" id="history"><h2>Recent Rescue History</h2><div style="overflow:auto"><table><thead><tr><th>Incident</th><th>Service</th><th>Priority</th><th>Callsign</th><th>Claim Time</th><th>Closed</th></tr></thead><tbody>{history_rows}</tbody></table></div></div>
<div class="card span12" id="config"><h2>Discord Configuration</h2><p class="muted">Only users with Manage Server permission can access this page. Role and incident-category changes are database-backed and picked up by the bot automatically.</p>
<form method="post" action="/guild/{guild_id}/config">
<input type="hidden" name="csrf" value="{csrf}">
<div class="grid">
<div class="span6"><label>Responder roles (may use controls on any incident)</label><select multiple name="responder_roles">{role_options}</select><p class="muted">Select one or more roles.</p>{service_html}</div>
<div class="span6">
<label>Request Assistance channel</label>{channel_select('request_channel', request_channel_id)}
<label>Live Dispatch Board channel</label>{channel_select('dispatch_channel', dispatch_channel)}
<label>Completed Rescue Log channel</label>{channel_select('log_channel', log_channel)}
<label>Active Incident category</label><select name="incident_category">{category_options}</select>
<p class="muted">Moving the Request or Dispatch channel posts a fresh panel/board in the newly selected channel. Old messages are left in place so nothing is deleted unexpectedly.</p>
</div></div>
<div style="margin-top:18px"><button class="btn" type="submit">Save Configuration</button> <a class="btn secondary" href="/">Back to Servers</a></div>
</form></div>
</div>"""
...
async def build_dispatch_embed(guild_id):
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            """SELECT incident_number,callsign,service,location,priority,status,primary_responder_id,created_at,channel_id
               FROM rescue_incidents WHERE guild_id=$1 AND status<>'closed'
               ORDER BY CASE priority WHEN 'critical' THEN 1 WHEN 'urgent' THEN 2 ELSE 3 END, incident_number ASC LIMIT 24""",
            guild_id,
        )
    fields = []
    if not rows:
...
async def save_config(request: Request, guild_id: int):
    ...
    for service in SERVICES:
        ids = [int(v) for v in form.getlist(f"service_{service}") if str(v).isdigit() and int(v) in valid_role_ids]
        service_map[service] = ids

    async with pool.acquire() as conn:
        previous = await conn.fetchrow(
            "SELECT request_channel_id FROM rescue_guild_settings WHERE guild_id=$1",
            guild_id,
        )
        previous_dispatch = await conn.fetchval(
            "SELECT channel_id FROM rescue_dispatch_boards WHERE guild_id=$1",
            guild_id,
        )
        async with conn.transaction():
            await conn.execute(
                """INSERT INTO rescue_guild_settings(guild_id,responder_role_ids,request_channel_id,incident_category_id,updated_at)
                   VALUES($1,$2,$3,$4,NOW())
                   ON CONFLICT(guild_id) DO UPDATE SET responder_role_ids=EXCLUDED.responder_role_ids,
                       request_channel_id=EXCLUDED.request_channel_id,incident_category_id=EXCLUDED.incident_category_id,updated_at=NOW()""",
                guild_id,
                responder_ids,
                request_channel,
                incident_category,
            ...
            for service, role_ids in service_map.items():
                await conn.execute(
                    """INSERT INTO rescue_service_role_settings(guild_id,service,role_ids,updated_at)
                       VALUES($1,$2,$3,NOW()) ON CONFLICT(guild_id,service)
            ...
            if log_channel:
                await conn.execute(
                    """INSERT INTO rescue_log_channels(guild_id,channel_id,updated_at) VALUES($1,$2,NOW())
    ...
    if dispatch_channel and dispatch_channel != previous_dispatch:
        message = await discord_post(
            ...
            {"embeds": [await build_dispatch_embed(guild_id)]},
        )
        async with pool.acquire() as conn:
            await conn.execute(
                """INSERT INTO rescue_dispatch_boards(guild_id,channel_id,message_id,updated_at) VALUES($1,$2,$3,NOW())
