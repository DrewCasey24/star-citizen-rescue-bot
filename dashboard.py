...
async def member_names(guild_id,user_ids):
    ...
    for uid in {int(v) for v in user_ids if v}:
        try:
            member=await discord_get(f"/guilds/{guild_id}/members/{uid}");user=member.get("user",{});names[uid]=member.get("nick") or user.get("global_name") or user.get("username") or "Discord User"
        except httpx.HTTPStatusError:names[uid]="Discord User"
    return names
...
async def guild_dashboard(request:Request,guild_id:int,saved:int=0):
    ...
    active_rows="".join(f'<tr><td><a class="incident-link" href="/guild/{guild_id}/incident/{r["incident_number"]}">RESCUE-{r["incident_number"]:04d}</a></td><td><span class="pill {"p1" if r["priority"]=="critical" else "p2" if r["priority"]=="urgent" else "p3"}">{esc(PRIORITIES.get(r["priority"],r["priority"]))}</span></td><td>{esc(STATUSES.get(r["status"],r["status"]))}</td><td>{esc(SERVICES.get(r["service"],r["service"]))}</td><td>{esc(r["callsign"])}</td><td>{esc(r["location"])}</td><td>{"Assigned" if r["primary_responder_id"] else "Unassigned"}</td></tr>' for r in active) or '<tr><td colspan="7" class="muted">No active incidents.</td></tr>'
    history_rows="".join(f'<tr><td><a class="incident-link" href="/guild/{guild_id}/incident/{r["incident_number"]}">RESCUE-{r["incident_number"]:04d}</a></td><td>{esc(SERVICES.get(r["service"],r["service"]))}</td><td>{esc(PRIORITIES.get(r["priority"],r["priority"]))}</td><td>{esc(r["callsign"])}</td><td>{duration((r["responded_at"]-r["created_at"]).total_seconds()) if r["responded_at"] else "—"}</td><td>{format_dt(r["closed_at"])}</td></tr>' for r in history) or '<tr><td colspan="6" class="muted">No completed incidents.</td></tr>'
...
async def incident_detail(request:Request,guild_id:int,incident_number:int):
    guild_info=require_guild_access(request,guild_id);await require_bot_installed(guild_id)
    async with pool.acquire() as conn:
        incident=await conn.fetchrow("SELECT incident_number,channel_id,requester_id,callsign,service,location,situation,priority,priority_changed_by,priority_changed_at,status,primary_responder_id,created_at,responded_at,arrived_at,backup_requested_at,closed_at,closed_by_id FROM rescue_incidents WHERE guild_id=$1 AND incident_number=$2",guild_id,incident_number)
        if not incident:raise HTTPException(status_code=404,detail="Incident not found.")
        responders=await conn.fetch("SELECT user_id,joined_at FROM rescue_incident_responders WHERE channel_id=$1 ORDER BY joined_at ASC",incident["channel_id"])
    user_ids=[incident["requester_id"],incident["primary_responder_id"],incident["priority_changed_by"],incident["closed_by_id"]]+[r["user_id"] for r in responders];names=await member_names(guild_id,user_ids)
    def who(uid,fallback="—"):return esc(names.get(uid,fallback)) if uid else fallback
    support=[who(r["user_id"],"Discord User") for r in responders if r["user_id"]!=incident["primary_responder_id"]];support_text=", ".join(support) if support else "None"
    events=[(incident["created_at"],"Incident Created",f'Request submitted by {who(incident["requester_id"],"Requester")}')]
    if incident["responded_at"]:events.append((incident["responded_at"],"Primary Responder Assigned",f'{who(incident["primary_responder_id"],"Responder")} accepted the call'))
    for r in responders:
        if r["user_id"]!=incident["primary_responder_id"]:events.append((r["joined_at"],"Responder Joined",f'{who(r["user_id"],"Responder")} joined the response team'))
    if incident["priority_changed_at"]:events.append((incident["priority_changed_at"],"Priority Changed",f'Priority changed to {esc(PRIORITIES.get(incident["priority"],incident["priority"]))}'+(f' by {who(incident["priority_changed_by"],"Responder")}' if incident["priority_changed_by"] else '')))
    if incident["arrived_at"]:events.append((incident["arrived_at"],"Arrived On Scene","Response team reported arrival on scene"))
    if incident["backup_requested_at"]:events.append((incident["backup_requested_at"],"Backup Requested","Additional responder support was requested"))
    if incident["closed_at"]:events.append((incident["closed_at"],"Incident Closed",f'Closed by {who(incident["closed_by_id"],"authorized user")}'))
    events.sort(key=lambda e:e[0]);timeline="".join(f'<div class="event"><div class="event-title">{esc(title)}</div><div>{text}</div><div class="event-meta">{format_dt(ts)}</div></div>' for ts,title,text in events)
    incident_id=f"RESCUE-{incident_number:04d}";pc="p1" if incident["priority"]=="critical" else "p2" if incident["priority"]=="urgent" else "p3";channel_button=f'<a class="btn" href="https://discord.com/channels/{guild_id}/{incident["channel_id"]}" target="_blank" rel="noopener">Open Discord Channel</a>' if incident["channel_id"] else ""
    body=f'''<div class="section-nav"><a href="/guild/{guild_id}">← Back to Dashboard</a></div><div class="grid"><div class="card span8"><div style="display:flex;justify-content:space-between;gap:16px;align-items:flex-start;flex-wrap:wrap"><div><div class="label">Incident</div><h1 style="margin-top:5px">{incident_id}</h1></div><div><span class="pill {pc}">{esc(PRIORITIES.get(incident['priority'],incident['priority']))}</span> <span class="pill">{esc(STATUSES.get(incident['status'],incident['status']))}</span></div></div><div class="kv" style="margin-top:22px"><div>Callsign</div><div><strong>{esc(incident['callsign'])}</strong></div><div>Service</div><div>{esc(SERVICES.get(incident['service'],incident['service']))}</div><div>Location</div><div>{esc(incident['location'])}</div><div>Requester</div><div>{who(incident['requester_id'],'Requester')}</div><div>Primary responder</div><div>{who(incident['primary_responder_id'],'Unassigned')}</div><div>Support responders</div><div>{support_text}</div><div>Created</div><div>{format_dt(incident['created_at'])}</div><div>Claimed</div><div>{format_dt(incident['responded_at'])}</div><div>Arrived</div><div>{format_dt(incident['arrived_at'])}</div><div>Closed</div><div>{format_dt(incident['closed_at'])}</div></div><div style="margin-top:20px">{channel_button}</div></div><div class="card span4"><h2>Response Timing</h2><div class="kv"><div>Claim time</div><div>{duration((incident['responded_at']-incident['created_at']).total_seconds()) if incident['responded_at'] else '—'}</div><div>On-scene time</div><div>{duration((incident['arrived_at']-incident['created_at']).total_seconds()) if incident['arrived_at'] else '—'}</div><div>Total duration</div><div>{duration((incident['closed_at']-incident['created_at']).total_seconds()) if incident['closed_at'] else 'Active'}</div></div></div><div class="card span12"><h2>Situation</h2><div class="situation">{esc(incident['situation'])}</div></div><div class="card span12"><h2>Incident Timeline</h2><p class="muted">Chronological incident activity reconstructed from the persisted rescue record and responder participation.</p><div class="timeline">{timeline}</div></div></div>'''
