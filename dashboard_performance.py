...
async def responder_performance(request: Request, guild_id: int, date_from: str = "", date_to: str = ""):
    ...
    rows_html = "".join(
        f'''<tr><td><strong>{esc(names.get(uid, "Discord User"))}</strong></td><td>{s.get("total_calls",0)}</td><td>{s.get("primary_calls",0)}</td><td>{max(0,s.get("total_calls",0)-s.get("primary_calls",0))}</td><td>{s.get("withdrawals",0)}</td><td>{base.duration(s.get("avg_claim"))}</td><td>{base.duration(s.get("avg_scene"))}</td></tr>'''
        for uid, s in ranked
...
<div class="card"><div style="display:flex;justify-content:space-between;gap:20px;align-items:flex-start;flex-wrap:wrap"><div><h2>Responder Performance</h2><p class="muted">Participation includes both primary and support response activity. Timing metrics apply to incidents where the responder is recorded as primary.</p></div><form method="get" action="/guild/{guild_id}/performance" style="display:flex;gap:14px;align-items:end;flex-wrap:wrap"><div style="min-width:150px"><label>From</label><input type="date" name="date_from" value="{esc(date_from)}"></div><div style="min-width:150px"><label>To</label><input type="date" name="date_to" value="{esc(date_to)}"></div><div style="display:flex;gap:8px;align-items:end"><button class="btn" type="submit">Apply</button><a class="btn secondary" href="/guild/{guild_id}/performance">Clear</a></div></form></div></div>
