...
<div class="card"><h2>Safe configuration cleanup</h2><p>This checks the server's saved channel, category, responder-role, and service-paging references against Discord. References to resources that no longer exist are removed. Valid configuration is left unchanged.</p><p class="muted">The repair does not delete Discord channels or roles. After cleanup, return to Settings to select replacements for anything that was removed.</p><form method="post" action="/guild/{guild_id}/repair-config"><input type="hidden" name="csrf" value="{csrf}"><button class="btn warn" type="submit">Repair Stale References</button> <a class="btn secondary" href="/guild/{guild_id}/health">Cancel</a></form></div>'''
...
async def repair_stale_config(request: Request, guild_id: int):
    ...
    async with base.pool.acquire() as conn:
        async with conn.transaction():
            settings = await conn.fetchrow("SELECT responder_role_ids,request_channel_id,incident_category_id FROM rescue_guild_settings WHERE guild_id=$1 FOR UPDATE", guild_id)
            if settings:
                old_roles = list(settings["responder_role_ids"] or [])
                new_roles = [rid for rid in old_roles if int(rid) in valid_roles]
                request_channel, category = settings["request_channel_id"], settings["incident_category_id"]
                ...
                repaired += len(old_roles)-len(new_roles) + int(bool(request_channel and new_request is None)) + int(bool(category and new_category is None))
                await conn.execute("UPDATE rescue_guild_settings SET responder_role_ids=$2,request_channel_id=$3,incident_category_id=$4,updated_at=NOW() WHERE guild_id=$1", guild_id,new_roles,new_request,new_category)
            for row in await conn.fetch("SELECT service,role_ids FROM rescue_service_role_settings WHERE guild_id=$1 FOR UPDATE", guild_id):
                old_roles=list(row["role_ids"] or []); new_roles=[rid for rid in old_roles if int(rid) in valid_roles]
                if new_roles != old_roles:
                    repaired += len(old_roles)-len(new_roles)
                    await conn.execute("UPDATE rescue_service_role_settings SET role_ids=$3,updated_at=NOW() WHERE guild_id=$1 AND service=$2", guild_id,row["service"],new_roles)
            dispatch=await conn.fetchrow("SELECT channel_id FROM rescue_dispatch_boards WHERE guild_id=$1 FOR UPDATE",guild_id)
            if dispatch and int(dispatch["channel_id"]) not in valid_text:
                repaired += 1; await conn.execute("DELETE FROM rescue_dispatch_boards WHERE guild_id=$1",guild_id)
            log_channel=await conn.fetchval("SELECT channel_id FROM rescue_log_channels WHERE guild_id=$1 FOR UPDATE",guild_id)
            if log_channel and int(log_channel) not in valid_text:
                repaired += 1; await conn.execute("DELETE FROM rescue_log_channels WHERE guild_id=$1",guild_id)
    return RedirectResponse(f"/guild/{guild_id}/health?repair={'done' if repaired else 'clean'}&count={repaired}",status_code=303)
...
def page_with_repair_shortcut(title, body, user=None):
    if title.startswith("System Health ·") and "Repair Configuration" not in body:
        ...
        if match:
            ...
            body = body.replace(marker, f'<a class="btn secondary" href="{root}/repair-config">Repair Configuration</a></div></div>\n<div class="health-grid">', 1)
